...
- 公開URL（project site）: `https://hanahanayaman.github.io/fandom-state-book/`
...
```bash
git clone https://github.com/hanahanayaman/fandom-state-book.git
...
4. 公開URL: `https://hanahanayaman.github.io/fandom-state-book/`

> このリポジトリは個人アカウント（`hanahanayaman`）の **public** リポジトリで、GitHub Pages を
> 無償で利用しています。`pathPrefix`（`.eleventy.js`）はリポジトリ名 `/fandom-state-book/` に
> 合わせています。別名のリポジトリやユーザー/組織サイトに置く場合はここを変更してください。
